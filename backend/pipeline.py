...
def run_cluster_branch(G_nx, label, args, results, st_status=None):
    ...
    # NOTE: CVD must run on the FULL graph — compression would remove leaf nodes
    # and produce a smaller/incorrect modulator that doesn't cover the full graph.
    print("  [Cluster Branch] Finding CVD modulator on full graph ...")
    if st_status: st_status.update(label="Finding CVD Modulator...", state="running")
    modulator_osm = set(find_cvd_minimal(G_nx)) if G_nx.number_of_nodes() > 0 else set()
...
def run_interval_branch(G_nx, label, args, results, st_status=None):
    ...
    # NOTE: IVD must run on the FULL graph — compression would remove leaf nodes
    # and produce a smaller/incorrect modulator that doesn't cover the full graph.
    print("  [Interval Branch] Finding IVD modulator on full graph ...")
    if st_status: st_status.update(label="Finding IVD Modulator...", state="running")

    adj_full = nx_to_adj(G_nx)
    modulator_osm = extract_ivd_modulator(adj_full) if adj_full else set()
...
def _find_cpp_exe():
    ...
    for name in ["block_cpp_compiler.exe", "block_cpp_compiler"]:
